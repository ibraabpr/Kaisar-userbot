    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**☣️ {ALIVE_NAME} Sedang OFF** {afk_since} **Yang Lalu.**\
                        \n➥ **Alasan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**☣️ {ALIVE_NAME} Masih OFF** {afk_since} **Yang Lalu.**\
                            \n➥ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"☣️ **{ALIVE_NAME} Sedang OFF** {afk_since} **Yang Lalu**.\
                        \n➥ **Alasan**: `{AFKREASON}`")
                else:
